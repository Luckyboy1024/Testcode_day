...
int main()
{
	...
	//放幻灯片，Sleep()函数调节播放速度
	for (i = 900; i <= 999; i++)//i 的范围转换为(.txt)文件的个数
	{
		sprintf(FileName,
			"D:\\1.My_Windows\\武器大师\\txtpicture\\(%ld).txt", i);
...
int main()
{
	...
	//system("C:\\Users\\ASUS\\Desktop\\字符动画.mp4");	//打开视频或音频文件，可以改为我们视频文件的地址
	Sleep(3000);						//窗口等待3秒后输出，可视需求更改时间，单位毫秒
	...
	for (i = 900; i <= 999; i++)//i 的范围转换为(.txt)文件的个数
	{
		...
		fread(hi, 1, 254000, in);//改 ,这是你一个TXT文件里的字符数量. 
